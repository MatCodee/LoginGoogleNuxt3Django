...
const fetchData = (accessToken: any)  =>   {
    /*
    const headers = {
    ...
    const response = fetch('http://127.0.0.1:8000' + '/rest-auth/google/',options);
    console.log("enviado la solicitud al backend")
    */
    const response =  fetch("http://localhost:8000/rest-auth/google/", {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
        },
...
<div class="w-full h-screen flex items-center justify-center">
  <div class="bg-white rounded-lg shadow-lg overflow-hidden w-full lg:max-w-lg">
        <div class="w-full p-8">
            <h2 class="text-2xl font-semibold text-gray-700 text-center">LOGIN</h2>
            <p class="text-xl text-gray-600 text-center">INGRESAR A LA APLICACION</p>
            <button class="flex items-center justify-center mt-4 w-full text-white rounded-lg shadow-md hover:bg-gray-100" @click="() => login()">
                <div class="px-4 py-3">
